#!/usr/bin/env python3
"""
Automated test for examples/menu/menu_with_no_icons using AT-SPI2.
...
# Launch the example
t.log("Launching menu_with_no_icons example...")
APP_NAME = unique_app_name()
proc = launch(['./examples/menu/menu_with_no_icons'], APP_NAME)

time.sleep(1)

t.log("Looking for window via AT-SPI...")
app, window = wait_for_window(APP_NAME, 'Menu Without Icons')
t.screenshot('Menu Without Icons')

if window is None:
    proc.kill()
    t.abort("Could not find Menu Without Icons window via AT-SPI")
...
t.check('Menu Without Icons' in (window.get_name() or ''),
        "Window title is 'Menu Without Icons'")
