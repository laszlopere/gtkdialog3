	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	switch (Type) {
		...
		case WIDGET_HELPBUTTON:
			widget = gtk_button_new_with_label("Help");
			attributeset_set_if_unset(Attr, ATTR_LABEL, "Help");
			break;
		case WIDGET_NOBUTTON:
			widget = gtk_button_new_with_label("No");
		...
		case WIDGET_YESBUTTON:
			widget = gtk_button_new_with_label("Yes");
			...
			while (act) {
				if (!input_is_shell_command(act)) {
					...
					if (stock_name != NULL) {
						...
						icon = gtk_image_new_from_icon_name(stock_name, stock_icon_size);
...
void widget_button_refresh(variable *var)
{
	...
	if (!initialised) {
		...
		if (var->Type == WIDGET_TOGGLEBUTTON) {
			...
			g_signal_connect(G_OBJECT(var->Widget), "enter-notify-event",
				G_CALLBACK(button_entered_attr),(gpointer)var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "leave-notify-event",
				G_CALLBACK(button_leaved_attr),(gpointer)var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "button-press-event",
				G_CALLBACK(button_pressed_attr),(gpointer)var->Attributes);
			g_signal_connect(G_OBJECT(var->Widget), "button-release-event",
