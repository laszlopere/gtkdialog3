	...
	AttributeSet *Attr, tag_attr *attr, gint Type)
{
	...
	switch (Type) {
		...
		case WIDGET_TERMINAL:
#if HAVE_VTE
			/* In VTE 2.91, padding is handled internally by the terminal */
			xpad = 0;
			ypad = 0;
...
/* Redundant
	if (attributeset_is_avail(Attr, ATTR_HEIGHT) && 
		...
		gtk_widget_set_size_request(scrolledwindow, 
			atoi(attributeset_get_first(&element, Attr, ATTR_WIDTH)),
			-1);
	} else if (GTK_IS_BOX(widget) || GTK_IS_BOX(widget)) {
		...
		gtk_widget_set_size_request(scrolledwindow, SW_DEFAULT_WIDTH, SW_DEFAULT_HEIGHT);
	}

	/$ Packing the widget, possibly with a viewport $/
	if (GTK_IS_TREE_VIEW(widget) || GTK_IS_BOX(widget) || GTK_IS_BOX(widget)) {
...
{
	g_warning("GVIM embedding (GtkSocket) is not supported in GTK3");
	return gtk_label_new("GVIM embedding not available in GTK3");
}
		...
		tag_attr      *tag_attributes)
{
	...
	switch (Widget_Type) {
		...
		case WIDGET_TERMINAL:
			...
			break;
		case WIDGET_SOURCEVIEW:
			Widget = widget_sourceview_create(Attr, tag_attributes, Widget_Type);
			scrolled_window = put_in_the_scrolled_window(Widget, Attr,
				tag_attributes, Widget_Type);
			push_widget(scrolled_window, WIDGET_SCROLLEDW);
			break;
