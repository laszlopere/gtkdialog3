	...
	const gchar *signal_name)
{
	...
	while (command) {
		...
		if (signal && g_ascii_strcasecmp(signal, signal_name) == 0) {
			...
			GDG_DEBUG("command=%s function=%s signal=%s signal_name=%s", command, function, signal, signal_name);
...
/* GtkWidget--->GtkContainer--->GtkTreeView */
				...
				if (strcasecmp(signal_name, "cursor-changed") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				} else if (strcasecmp(signal_name, "selection-changed") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				} else if (strcasecmp(signal_name, "row-activated") == 0) {
					execute = widget_signal_executor_eval_condition(condition);
				}
...
/* GtkWidget--->GtkContainer--->GtkBin--->GtkItem--->GtkMenuItem--->GtkCheckMenuItem */
			...
			GDG_DEBUG("argument=%s", argument);
			GDG_DEBUG("not=%i", not);
			GDG_DEBUG("line=%s", line);
			GDG_DEBUG("state=%i", state);
		...
		for (count = 0; count < 2; count++) {
			...
			if (((!count && (value = get_tag_attribute(var->widget_tag_attr,
				...
				if ((file = g_file_new_for_path(find_pixmap(filename)))) {
					...
					GDG_DEBUG("file=%p monitor=%p", file, monitor);
